...
class AuthenticationController extends Controller
{
    public function register(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $mailDetails["body"] = "Your One Time (OTP) for verification is " . $oneTimeOtp . " Do\'t share your OTP with others.";
    ...
    public function forgotPassword(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $findUser = User::where('email', $request->email)->first();
            if (!$findUser) {
                return response()->json(['status' => "error", 'message' => "Email not found, please register first."]);
            } else {
                $oneTimeOtp = rand(1000, 9999);
                $mailDetails = [];
                $mailDetails['title'] = "Hi, $request->full_name . Welcome to " . env("APP_NAME");
                $mailDetails["body"] = "Your One Time (OTP) for reset password is " . $oneTimeOtp . " Do\'t share your OTP with others.";
                $sendMail = $this->sendOTPmail($request->email, $mailDetails);
                if ($sendMail) {
                    $findUser->otp = $oneTimeOtp;
                    $findUser->save();
                    return response()->json(['status' => "success", 'errors' => "A code for reset password sent to your email."]);
                } else {
                    return response()->json(['status' => "error", 'message' => "Something wend wrong please try again later."]);
                }
            }
        }
    }
    ...
    public function setNewPassword(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $findUser = User::where('email', $request->email)->first();
            if (!$findUser) {
                return response()->json(['status' => "error", 'message' => "User not found, prease register first. "]);
            } else {
                if ($findUser->otp == $request->otp) {
